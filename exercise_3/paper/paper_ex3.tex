...
\section{Copycat Networks for CNN}
    ...
    \subsection{Microsoft Azure Emotion API}
        As our first target model we used the publicly available Microsoft Azure Emotion API. For an input image this API returns a percentage vector of emotions for each face in the image. For each of the following emotions a value is assigned: 
        %This API classifies the emotion a face portrays, in the form of a percentage vectors that assigns values to the following emotions:
            \begin{itemize}
                ...
                \item Surprise
            \end{itemize}
        For example the face in Figure~\ref{fig:happy_face} from the publicly available KDEF dataset returned a vector that assigned a percentage of $0.999$ to happiness $0.001$ to surprise and $0$ to everything else.
            \begin{figure}[h!]
                    \centering      \includegraphics[scale=0.2]{exercise_3/paper/images/AM18HAS.JPG}
                    \caption{Face classified as happy by Azure}
        ...
        We started with stealing the model behind this API as it was already considered in the paper. Hence, we can assess whether the performance of our copied classifier is similar to the performance of the stolen classifier in the paper.
        
        Originally, we wanted to add instances, which are not part of the problem domain to our fake dataset. However, as Azure does not classify images in which it does not detect any face, this was not possible. 
        
        Testing the Azure Classifier using the pre-labeled data from the Kaggle Competion we got an accuracy of $0.76$. \todo{in Evaluation}
        
    \subsection{Evaluation 1}
        We used the Karolinska Directed Emotional Faces (KDEF) and Averaged Karolinska Directed Emotional Faces (AKDEF) dataset to query Azure as a Blackbox. As Azure did not recognise some of the faces we added No face detected as an additional class. \todo{no face detected in 3.2}
    ...
    \subsection{Second Data Set}
        Model: https://www.kaggle.com/c/dogs-vs-cats/data
